...
uint8_t *Slurp::readfile(const char *filename, std::size_t *len, bool rw,
...
void Slurp::writefile(const char *filename, uint8_t *buf, std::size_t len) {
...
uint8_t *Slurp::mapMemory(std::size_t size, const void *preferred_addr) {
    auto *buf =
        (uint8_t *)mmap((void *)preferred_addr, size, PROT_READ | PROT_WRITE,
